...
**Stale-cwd guard:** `init.lua` checks `vim.uv.fs_realpath(".")` before anything else loads, and `lua/config/autocmds.lua` re-checks on every `DirChanged` — both `vim.cmd.cd(vim.env.HOME)` if the process's working directory no longer exists on disk (e.g. the shell nvim was launched from was sitting in a directory that got deleted). Without this, `vim.uv.cwd()` and plenary's lazily-resolved `Path._cwd` return nil, which crashes Telescope's previewer (`filetype_detect` → `Path:is_file()`) and Harpoon's default per-project key (`vim.loop.cwd()` used directly as a table index) with confusing errors nowhere near the real cause. Harpoon's `key` is also overridden directly in `lua/plugins/harpoon.lua` (`vim.uv.cwd() or vim.env.HOME`) as defense in depth. `vim.fn.getcwd()` (used by oil.nvim/conform.nvim/lazygit.nvim) is a cached string, not a live syscall, so it isn't affected by this class of bug.

**Colorschemes:** `cerne` (native `colors/cerne.lua`, dark) and `cerne-light` (`colors/cerne-light.lua`) use the brand doc's own tokens for each polarity (oxblood stands in for amber on light — same rule the guidelines give for web). Both went through a WCAG contrast pass — see the header comment in each file for what was wrong and why (in short: blink.cmp's ghost text defaulting to a near-invisible `NonText`, float borders and completion-selection rows barely distinguishable from their surroundings, oxblood used as foreground text where the guidelines themselves say not to, and `SnacksDashboardDesc`/`Key`/`Icon`/`Footer`/`Special` weren't defined in either colorscheme file at all — only in a since-fixed `autocmds.lua` fallback that unconditionally applied dark values, making cerne-light's dashboard text invisible).

By default nvim follows the macOS system appearance on startup (`lua/plugins/colorschemes.lua`, checks `defaults read -g AppleInterfaceStyle`) and picks `cerne`/`cerne-light` accordingly. Any explicit pick via `<leader>uC` (Telescope colorscheme picker with preview) overrides this and stays sticky across restarts — written to `stdpath("state")/cerne_colorscheme` on every `ColorScheme` event. Run `:CerneThemeAuto` to clear an explicit pick and resume following the system. catppuccin, rose-pine, tokyonight, gruvbox, kanagawa, nightfox, everforest, onedark, nord, and github-nvim-theme are also installed as alternates — picking one of those is just as sticky, `:CerneThemeAuto` returns to system-following `cerne`/`cerne-light` specifically.

An already-open nvim also updates live (no restart, no manual command) when macOS Dark Mode toggles — a `launchd` LaunchAgent in the `dotfiles` repo (`bin/cerne-theme-watch.sh`) remote-sends `:CerneThemeAuto` to every running instance's auto-started RPC socket (`v:servername`), but only ones currently in "auto" mode. That watcher lives in `dotfiles`, not here, since it also drives tmux.
